    ...
    external_neighbors=FALSE, BNPARAM = KmknnParam(), BPPARAM = SerialParam())
{
    ...
    if (!transposed) {
        ...
        xshared <- .get_mat_for_reddim(xshared, subset_row=subset_row, ntop=ntop, scale=scale)
        yshared<- .get_mat_for_reddim(yshared, subset_row=subset_row, ntop=ntop, scale=scale)
    }
                                                            ...
                                                            exprs_values="logcounts", dimred=NULL, use_dimred=NULL, n_dimred=NULL)
{
    ...
    satsne_out <- .calculate_satsne(matx,maty,shared_feats, transposed=!is.null(dimred), pca=pca, ...)

    satsne_out$Y1 <- t(satsne_out$Y1) # Transposing back.
    satsne_out$Y2 <- t(satsne_out$Y2) # Transposing back.

    reducedDim(x,"SATSNE") <- satsne_out$Y1
    reducedDim(y,"SATSNE") <- satsne_out$Y2
